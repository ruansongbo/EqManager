...
{
    public partial class frmFixLook : Form
    {
        public frmFixLook()
        {
            InitializeComponent();
        ...
        {
            this.dbgFix.Columns[1].Frozen = true;//第二列前的列都被冻结
        }
        ...
        {
            DataTable dt = DgvMgr.GetTable(this.dbgFix);
        ...
        {
            frmPrintOptions dlg = new frmPrintOptions(dbgFix);
            dlg.ShowDialog();
        }
        ...
        private void toolbtnSearch_Click(object sender, EventArgs e)
        {
            if (!query_flag)
            {
                ...
                foreach (DataGridViewColumn c in dbgFix.Columns)
                {
                    ...
                    this.dbgFix.Controls.Add(btnSort);
                    btnSort.Visible = true;
                    btnSort = this.sortButtons[c.HeaderText];
                    btnSort.Column = dbgFix.Columns[c.Index];
        ...
        private void btnSort_Click(object sender, EventArgs e)
        {
            ...
            {
                frmSort = new ZCGridSortForm(this.dbgFix, this.SortsString, this.sortButtons, this.OrderByString);
        ...
        private void SetupSortButtonsPosition()
        {
            ...
            {
                if (this.dbgFix.Columns.Contains(columnName) == false || this.sortButtons.ContainsKey(columnName) == false) continue;
                DataGridViewColumn col = this.dbgFix.Columns[columnName];
                ZCGridColumnSortButton btnSort = this.sortButtons[columnName];
                Rectangle r = this.dbgFix.GetCellDisplayRectangle(col.DisplayIndex, -1, false);
        ...
        private void dbgFix_ColumnWidthChanged(object sender, DataGridViewColumnEventArgs e)
        {
            SetupControlsPosition();
        }

        private void dbgFix_Scroll(object sender, ScrollEventArgs e)
        {
            SetupControlsPosition();
        }

        private void dbgFix_SizeChanged(object sender, EventArgs e)
        {
            SetupControlsPosition();
        }
